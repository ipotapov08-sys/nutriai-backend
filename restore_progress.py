...
# Проверяем, есть ли checkin
if '@app.post("/checkin")' not in content:
    print('Чекин отсутствует — добавляем')
...
async def daily_checkin(request: Request):
    try:
        ...
        if not user_id: return {"status": "error", "message": "user_id required"}
        conn = sqlite3.connect(DB_PATH)
        today_str = datetime.date.today().isoformat()
        conn.execute("INSERT OR REPLACE INTO daily_checkins (user_id, date, weight, mood, energy_level, sleep_hours, water_ml, notes, completed) VALUES (?,?,?,?,?,?,?,?,1)", (user_id, today_str, data.get("weight"), data.get("mood"), data.get("energy_level"), data.get("sleep_hours"), data.get("water_ml"), data.get("notes")))
        conn.commit()
        checkins = conn.execute("SELECT date FROM daily_checkins WHERE user_id=? ORDER BY date DESC LIMIT 30", (user_id,)).fetchall()
        streak = 1 if checkins else 0
        for i in range(len(checkins)-1):
            d1 = datetime.date.fromisoformat(checkins[i][0])
            d2 = datetime.date.fromisoformat(checkins[i+1][0])
...
async def get_weekly_report(user_id: int = Query(...)):
    ...
    avg_cal = sum(r[1] for r in nutrition) / max(len(nutrition), 1) if nutrition else 0
    ...
    return {"week": f"{monday} - {today}", "checkin_days": checkin_days, "compliance": compliance, "avg_calories": round(avg_cal, 1), "weight_change": weight_change, "recommendations": recs, "nutri_message": "🥑 Ты молодец!" if compliance >= 70 else "🥑 Нутри грустит..."}
'''

    old = '@app.on_event("startup")'
    content = content.replace(old, checkin + '\n' + old)
    with open(path, 'w', encoding='utf-8') as f:
        f.write(content)
    print('Эндпоинты прогресса восстановлены')
else:
    print('Чекин уже есть')
